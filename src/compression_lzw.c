...
compression_result_t compress_lzw(const unsigned char *input, size_t input_size) {
	...
	size_t codes_capacity = input_size * 2;
	if (codes_capacity < 1) {
		codes_capacity = 1;
	}
	...
	for (size_t i = 1; i < input_size; ++i) {
		...
		if (next_code != -1) {
			...
			if (code_count >= codes_capacity) {
				codes_capacity *= 2;
				uint16_t *new_codes = (uint16_t *)realloc(codes, codes_capacity * sizeof(uint16_t));
				if (!new_codes) {
					free(codes);
					result.error = -3;
					return result;
...
compression_result_t decompress_lzw(const unsigned char *input, size_t input_size) {
	...
	unsigned char sequence[LZW_MAX_DICT_SIZE];
	...
	for (uint32_t i = 1; i < code_count; ++i) {
		...
		} else if (new_code == dict_size && dict_size < LZW_MAX_DICT_SIZE) {
